...
ArmyKnights::ArmyKnights(const string & file_armyknights): neededCondition{}, hadFightWithUlti{false}
{
...
BaseKnight* ArmyKnights::lastKnight() const {
    if (hadFightWithUlti) return lastKnightAfterFightWithUltimecia();
...
bool ArmyKnights::fightMinorMonster(int eventOrder, int monsterType)
{
    ...
    knight->fight(monster);
    if (knight->getHp() <= 0) deleteKnightAt(numOfKnight-1);
    else
    {
        ...
        {
            knight->setGil(999);
            passGilToPreviousKnight(gil - 999, numOfKnight - 1);
        }
    }
...
BaseKnight* ArmyKnights::lastKnightAfterFightWithUltimecia() const  {
    for (int i = initNumOfKnights - 1; i >= 0; --i) {
